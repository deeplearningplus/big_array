...
lib.write_big_array.argtypes = [ctypes.c_char_p, ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t]
lib.write_big_array.restype = ctypes.c_int

lib.read_big_array.argtypes = [ctypes.c_char_p, ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t]
...
lib.read_at_index.argtypes = [ctypes.POINTER(MappedArray), ctypes.c_size_t, ctypes.POINTER(ctypes.c_uint8)]
lib.read_at_index.restype = ctypes.c_int

lib.read_range.argtypes = [ctypes.POINTER(MappedArray), ctypes.c_size_t, ctypes.c_size_t, ctypes.POINTER(ctypes.c_uint8)]
lib.read_range.restype = ctypes.c_int

lib.read_range_simd.argtypes = [ctypes.POINTER(MappedArray), ctypes.c_size_t, ctypes.c_size_t, ctypes.POINTER(ctypes.c_uint8)]
lib.read_range_simd.restype = ctypes.c_int

lib.read_range_simd_parallel.argtypes = [ctypes.POINTER(MappedArray), ctypes.c_size_t, ctypes.c_size_t, ctypes.POINTER(ctypes.c_uint8)]
...
def example_usage():
    ...
    #array_size = int(1e9)
    #big_array = rng.randint(0, 256, array_size, dtype=np.uint8)
    #test_filename = "big_array_c.bin"
    #if lib.write_big_array(test_filename.encode('utf-8'), big_array.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), array_size) != 0:
    ...
    #read_array = np.zeros(array_size, dtype=np.uint8)
    #if lib.read_big_array(filename.encode('utf-8'), read_array.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), array_size) != 0:
    ...
    # Read with np.memmap
    tic = time.time()
    a = np.memmap(filename, dtype=np.uint8, mode="r")
    ...
    if lib.read_range(marray, start, length, buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8))) != 0:
    ...
    # Use SIMD read range
    tic = time.time()
    if lib.read_range_simd(marray, start, length, buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8))) != 0:
    ...
    tic = time.time()
    if lib.read_range_simd_parallel(marray, start, length, buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8))) != 0:
        print("Error reading range of values with SIMD parallel")
        lib.close_mmap_array(marray)
        return
    #print(f"Values from {start} to {start + length} (SIMD parallel): {buffer}")
