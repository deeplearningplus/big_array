 ...
 * @return The number of elements (size in bytes divided by 2 since each element is uint16)
 *         or -1 on error
 */
size_t get_packed_array_size(const char *filename) {
    ...
    close(fd);
    return st.st_size / sizeof(uint16_t);  // For uint16, number of elements equals file size in bytes divided by 2
}
 ...
 */
int write_big_array(const char *filename, const uint16_t *arr, size_t size) {
    ...
    size_t written = fwrite(arr, sizeof(uint16_t), size, file);
    if (written != size) {
 ...
 * @return 0 on success, -1 on failure
 */
int read_big_array(const char *filename, uint16_t *arr, size_t size) {
    if (filename == NULL || arr == NULL || size == 0) {
        fprintf(stderr, "Invalid arguments\n");
        return -1;
    }
    ...
    if (file_size != size * sizeof(uint16_t)) {
        fprintf(stderr, "File size (%ld) does not match expected size (%zu)\n",
                file_size, size * sizeof(uint16_t));
        fclose(file);
        return -1;
    }

    // Read the entire file into the provided array
    size_t read_size = fread(arr, sizeof(uint16_t), size, file);
    if (read_size != size) {
 ...
 */
typedef struct {
    uint16_t *data;     // Pointer to the memory-mapped data
...
MappedArray* mmap_big_array(const char *filename) {
    ...
    marray->size = sb.st_size / sizeof(uint16_t);

    // Memory map the file
    marray->data = (uint16_t*)mmap(NULL, sb.st_size,
...
void close_mmap_array(MappedArray *marray) {
    if (marray != NULL) {
        if (marray->data != NULL && marray->data != MAP_FAILED) {
            munmap(marray->data, marray->size * sizeof(uint16_t));
        }
        if (marray->fd != -1) {
            close(marray->fd);
        }
 ...
 * @return 0 on success, -1 on failure
 */
int read_at_index(const MappedArray *marray, size_t index, uint16_t *value) {
...
int read_range(const MappedArray *marray, size_t start,
    ...
    memcpy(buffer, marray->data + start, length * sizeof(uint16_t));
    return 0;
}
 ...
 */
int read_at_index_simd(const MappedArray *marray, size_t index, uint16_t *value) {
    if (marray == NULL || marray->data == NULL || value == NULL) {
        return -1;
    }
    if (index >= marray->size) {
...
int read_range_simd(const MappedArray *marray, size_t start,
    ...
    const uint16_t *src = marray->data + start;
    uint16_t *dst = buffer;
    size_t i = 0;

    // Process 16 elements (32 bytes) at a time using AVX2
    ...
    if (i < length) {
        memcpy(dst + i, src + i, (length - i) * sizeof(uint16_t));
    }
...
int read_range_simd_parallel(const MappedArray *marray, size_t start,
    ...
    const size_t min_chunk_size = 32 * 1024 / sizeof(uint16_t);

    // Calculate chunk size based on length
    size_t chunk_size = length / omp_get_max_threads();
    chunk_size = (chunk_size + 15) & ~15; // Round up to multiple of 16 elements
    ...
    #pragma omp parallel
    {
        ...
        if (thread_start < length) {
            // Align thread_end to 16-element boundary for AVX2
            thread_end = (thread_end > length) ? length : thread_end;
            size_t thread_i = thread_start;

            // Process 128 bytes (64 elements) at a time using AVX2
            for (; thread_i + 64 <= thread_end; thread_i += 64) {
                // Load and store first 32 elements
                __m256i data1 = _mm256_loadu_si256((const __m256i*)(src + thread_i));
                _mm256_storeu_si256((__m256i*)(dst + thread_i), data1);

                // Load and store next 32 elements
            ...
            for (; thread_i + 16 <= thread_end; thread_i += 16) {
            ...
            if (thread_i < thread_end) {
                memcpy(dst + thread_i, src + thread_i, (thread_end - thread_i) * sizeof(uint16_t));
            }
        }
    }
